                   ...
                   int cmdShow)
{
	...
	theMoon.initialise(moonTexture.getTexture(), glm::vec3(0, 0, 8), glm::vec3(0, 0, 0));
	float moonRotSpeed = 5.0f;
	GLfloat moonOrbit = 1.0f;

	theSun.initialise(sunTexture.getTexture(), glm::vec3(5, 0, 12), glm::vec3(0, 0, 0));
	float sunRotSpeed = 0.5f;
   ...
   //This is the mainloop, we render frames until isRunning returns false
	while (pgmWNDMgr->isWNDRunning())
    {
		...
		sunLight.lightOn();
		glTranslatef(0, 0, 0);
		theSun.setRotAngle(0.0f);
		theSun.prepare(theSun.getRotAngle());
		sunMaterial.useMaterial();
		theSun.render(theEarth.getRotAngle());
